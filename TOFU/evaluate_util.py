...
def get_kl_divergence(model, oracle_model, eval_dataloader):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            # labels = labels.to(oracle_model.device)
...
def get_masked_kl_divergence(model, oracle_model, eval_dataloader):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            outputs_logits = outputs.logits.clone()  # Shape: [batch_size, seq_length, vocab_size]
            outputs_logits_oracle = outputs_oracle_model.logits.clone() 

            batch_size, seq_len = input_ids[:, 1:].size()

            # Create index tensors for batch and sequence
            batch_idx = torch.arange(batch_size).unsqueeze(1).expand(-1, seq_len).to(model.device)  # (batch_size, seq_len)
            seq_idx = torch.arange(seq_len).unsqueeze(0).expand(batch_size, -1).to(model.device)    # (batch_size, seq_len)

            # Mask logits for the current model by setting corresponding token logits to a large negative value
            outputs_logits[batch_idx, seq_idx, input_ids[:, 1:]] = float(-1e4)

            # Mask logits for the oracle model in the same manner
            outputs_logits_oracle[batch_idx, seq_idx, input_ids[:, 1:]] = float(-1e4)

            # Calculate log probabilities for KL divergence calculation
            probs = F.log_softmax(outputs_logits, dim=-1)  # Shape: [batch_size, seq_length, vocab_size]
            probs_oracle = F.log_softmax(outputs_logits_oracle, dim=-1)  # Shape: [batch_size, seq_length, vocab_size]

            # Flatten the tensors for batch-wise KL divergence calculation
            probs = probs.view(-1, outputs_logits.shape[-1])  # Shape: [batch_size * seq_length, vocab_size]
            probs_oracle = probs_oracle.view(-1, outputs_logits_oracle.shape[-1])  # Shape: [batch_size * seq_length, vocab_size]

            # Calculate KL divergence and reduce by 'batchmean'
            kl_divergence = nn.functional.kl_div(probs, probs_oracle, reduction='batchmean', log_target=True)

            # Append the KL divergence to the output list
            kl_outputs.append(kl_divergence.item())

    kl_mean = torch.tensor(kl_outputs).mean()
...
def main(cfg):
    ...
    for i, (folder, split, question_key, answer_key, eval_task, base_answer_key, perturbed_answer_key) in enumerate(zip(cfg.data_path, cfg.split_list, cfg.question_key, cfg.answer_key, cfg.eval_task, cfg.base_answer_key, cfg.perturbed_answer_key)):
        ...
        # kl_divergence_log = get_masked_kl_divergence(model, oracle_model, eval_dataloader)
        # print(kl_divergence_log)
        # print('--------------')
        ...
        # kl_divergence_log = get_kl_divergence(model, oracle_model, eval_dataloader)
        # eval_logs['kl_divergence'] = kl_divergence_log
...
if __name__ == "__main__":
    cfg_dict = OmegaConf.load("config/eval_everything.yaml")
    cfg = OmegaConf.create(cfg_dict)
    cfg['split'] = 'forget05_perturbed'
    cfg['model_path'] = ''
    cfg['save_dir'] =  f''
    main(cfg)
