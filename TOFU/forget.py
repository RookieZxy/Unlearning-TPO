...
from dataloader import CustomTrainerForgetting, custom_data_collator_forget, PCL_Trainer
import torch
# torch.cuda.set_per_process_memory_fraction(0.82, device=torch.device('cuda:0'))
...
def main(cfg):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_id, use_auth_token=HUG_TOKEN, use_fast=True)
    ...
    if cfg.forget_loss in ["dpo","dpo_gpt","dpo_grad_diff"]:
        torch_format_dataset = TextForgetDatasetDPOQA(data_path, 
                                                    tokenizer=tokenizer, 
                                                    model_family = cfg.model_family, 
                                                    max_length=max_length, 
                                                    split=cfg.split,
                                                    fill_mask=cfg.fill_mask,
    ...
    trainer.save_loss_log_csv(cfg.save_dir)
